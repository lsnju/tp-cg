...
<mapper namespace="${tableMo.daoMo.apiPackage}.${tableMo.daoMo.apiName}">
    ...
    <insert id="add" <#if tableMo.idColumn??><#if tableMo.idColumn.autoIncrement>useGeneratedKeys="true" keyProperty="${tableMo.idColumn.propName}" keyColumn="${tableMo.idColumn.columnName}" </#if></#if>parameterType="${tableMo.daoMo.modelPackage}.${tableMo.daoMo.modelName}">
    <#if tableMo.idColumn??>
        <#if tableMo.idColumn.autoIncrement>
        <#else>
        <selectKey keyProperty="${tableMo.idColumn.propName}" resultType="${tableMo.idColumn.propType.type}" order="BEFORE">
            <![CDATA[
            select nextval('SEQ_${tableMo.tableName}')
            ]]>
        </selectKey>
        </#if>
    </#if>
